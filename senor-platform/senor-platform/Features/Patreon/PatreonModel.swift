...
extension PatreonViewModel {
    func configureForPreview(
        ...
        identity: PatreonIdentityResponse? = nil,
        campaign: PatreonCampaign? = nil,
        posts: [PatreonPost] = [],
        ...
        if let identity = identity { self.identity = identity }
        if let campaign = campaign { self.campaign = campaign }
        self.posts = posts
        self.members = members
        self.isLoadingProfile = isLoadingProfile
        if let error = profileError { self.profileError = error }
        if let error = postsError { self.postsError = error }
    }
}
#endif
